...
export default function AdminHealthPage() {
  ...
  <Panel title="Lixeira recuperável">{trash.length === 0 ? <p className="text-sm text-gray-500">Nenhum conteúdo arquivado.</p> : trash.map((item) => <div key={item.id} className="flex items-center justify-between gap-3 border-t border-white/10 py-3"><div><strong className="text-sm text-white">{String(item.snapshot.title || item.content_id)}</strong><p className="text-xs text-gray-500">Expira em {new Date(item.expires_at).toLocaleDateString("pt-BR")}</p></div><button onClick={() => void restore(item.id)} className="min-h-9 border border-brand-orange/50 px-3 text-xs font-bold text-brand-orange">Restaurar</button></div>)}</Panel>
