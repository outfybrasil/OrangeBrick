...
export default function BrickboardPage() {
  ...
  useEffect(() => {
    ...
    async function fetchPosts() {
      try {
        ...
        setError(err instanceof Error ? err.message : "Erro ao carregar o Brickboard");
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const stats = usePostStats(posts.map((post) => post.id));
  const rankedPosts = useMemo(
    () => [...posts].sort((a, b) => (stats[b.id]?.reactions.hype || 0) - (stats[a.id]?.reactions.hype || 0)),
    [posts, stats]
  );
  ...
  return (
    <>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {!isLoading && rankedPosts.length > 0 && (
          <div className="brickboard-grid">
            {rankedPosts.map((post) => {
              const hype = stats[post.id]?.reactions.hype || 0;
              const baseSpan = 22;
                  ...
                  <div className="p-4 space-y-2">
                    ...
                    <div className="flex items-center gap-3 pt-2 text-[10px] font-mono text-gray-500 border-t border-brand-orange-muted/10">
                      {Object.entries(REACTION_ICONS).map(([type, icon]) => {
                        const count = stats[post.id]?.reactions[type as "hype" | "flop" | "salty"] || 0;
