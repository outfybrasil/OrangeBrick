...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const results = query.length >= 2 ? await Promise.all([
    ...
    supabase.from("topics").select("*").eq("is_active", true).or(`name.ilike.${pattern},description.ilike.${pattern}`).limit(8),
    supabase.from("release_radar_items").select("*").eq("is_active", true).ilike("game", pattern).limit(8),
  ...
  <div className="mt-10 grid gap-10 lg:grid-cols-2">
    ...
    <ResultSection title="Assuntos e lançamentos">{topics.map((topic) => <Link key={topic.id} href={`/assuntos/${topic.id}`} className="block border-t border-white/10 py-4 text-sm font-bold hover:text-brand-orange">{topic.name}</Link>)}{releases.map((release) => <Link key={release.id} href={`/lancamentos#${release.id}`} className="block border-t border-white/10 py-4 text-sm font-bold hover:text-brand-orange">{release.game} <span className="font-normal text-gray-500">· {release.release_label}</span></Link>)}</ResultSection>
