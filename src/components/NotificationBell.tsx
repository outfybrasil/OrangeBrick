    ...
    <div className="relative inline-block">
      <button
        ...
        className={`relative flex items-center justify-center w-9 h-9 rounded-xl border transition-all cursor-pointer ${
          subscribed
            ? "bg-brand-orange/15 text-brand-orange border-brand-orange/40 shadow-[0_0_12px_rgba(255,94,0,0.25)] hover:bg-brand-orange/25"
            : "bg-card-slate/80 text-gray-400 border-brand-orange-muted/20 hover:border-brand-orange/40 hover:text-white hover:bg-card-slate"
        } disabled:opacity-50`}
        title={subscribed ? "Notificações Ativas (Clique para desativar)" : "Ativar Notificações do Portal"}
      ...
      >
        {loading ? (
          ...
          <div className="relative flex items-center justify-center">
            ...
            {subscribed && (
              <span className="absolute -top-1 -right-1 w-2 h-2 rounded-full bg-brand-orange shadow-[0_0_6px_#FF5E00]" />
            )}
          </div>
        )}
      </button>
