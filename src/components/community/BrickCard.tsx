...
export function BrickCard({ post, onReaction, onDeletePost, onSharePost, onAddComment, onDeleteComment, getComments }: BrickCardProps) {
  ...
  return (
    <article className="bg-card-slate/60 border border-brand-orange-muted/15 rounded-2xl p-5 shadow-lg hover:border-brand-orange-muted/30 transition-all space-y-4 relative group/card">
      ...
      {post.shared_post && (
        <div className="p-3.5 rounded-xl bg-[#0D0F14]/80 border border-emerald-500/20 space-y-2">
          <div className="flex items-center gap-2">
            <svg className="w-3.5 h-3.5 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
            </svg>
            <span className="text-[10px] font-subtitle font-bold text-emerald-400 uppercase tracking-wider">
              Republicado de {post.shared_post.original_author_name}
            </span>
          </div>
          <div className="flex gap-2.5 items-start">
            <img
              src={post.shared_post.original_author_avatar}
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[11px] text-gray-300 font-body leading-relaxed mt-1 whitespace-pre-line">
        ...
        <div className="pt-4 border-t border-brand-orange-muted/15 space-y-3">
          ...
          <form onSubmit={handleSubmitShare} className="flex gap-2">
            ...
            <button
              ...
              className="px-4 py-2 text-xs font-subtitle font-bold bg-emerald-600 text-white rounded-xl disabled:opacity-40 hover:bg-emerald-500 transition-all cursor-pointer shrink-0"
            >
